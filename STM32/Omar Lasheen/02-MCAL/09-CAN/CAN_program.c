...
void CAN_voidInit(void)
{
    ...
    GPIO_voidSetPinDirection(GPIOB, 8, INPUT_FLOATING); //RX
    GPIO_voidSetPinDirection(GPIOB, 9, OUTPUT_SPEED_2MHZ_AFPP); //TX
    /**  exit CAN sleep mode **/
...
void CAN_voidInitInterput(uint8 local_u8InterputNum)
{ /* init interrput*/
	NVIC_voidEnableInterrupt(local_u8InterputNum);
	NVIC_voidSetPendingFlag(local_u8InterputNum);
}
/** Disanable NVIC Interrupt **/
void CAN_voidDeiniterputInit(uint8 local_u8InterputNum)
{ /* deinit interrput*/
	NVIC_voidDisableInterrupt(local_u8InterputNum);
	NVIC_voidClearPendingFlag(local_u8InterputNum);
}
